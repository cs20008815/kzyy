...
!function (a) {
	...
	c.events = function () {
		...
		c.on(h.oclear, "click", function () {
			var f = ""
			//c.elem[h.elemv] = "" || "function" == typeof c.options.choose && c.options.choose(c.elem[h.elemv]);
			c.elem[h.elemv] = f,f || (c.close(), "function" == typeof c.options.choose && c.options.choose(f));
//			c.options.choose && c.options.choose(f);
	//		c.creation(c.elem[h.elemv]);
			c.close()
		}),
		h.otoday = b("#laydate_today"),
		c.on(h.otoday, "click", function () {
			var f = laydate.now(0, c.options.format);
			c.elem[h.elemv] = laydate.now(0, c.options.format),f || (c.close(), "function" == typeof c.options.choose && c.options.choose(f))
			c.options.choose && c.options.choose(f)
			c.close()
		}),
		...
		c.on(d, "mouseup", function () {
			...
			b = b || a.event;
			var d = b.keyCode;
			13 === d && c.elem != null && c.creation([c.ymd[0], c.ymd[1] + 1, c.ymd[2]])
		})
	},
	c.init = function () {
		c.use("need"),
